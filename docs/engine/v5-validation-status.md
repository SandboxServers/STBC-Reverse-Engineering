...
| 10 | decompiled-functions.md | Leaf: per-function notes (net/checksum/event) | function-map.md, event-system-architecture.md | verified (2026-05-28) — fourth verified doc; 34 addresses + 5 decompile-verified behavioral spot-checks + 7 cross-doc anchors all PASS; zero corrections; cleanest validation in the engine family |
...
- **Moved (Group B → Internal C++ classes section):** ~15 real C++ classes that lack bare class-name strings because they are not SWIG-bound — TGStream, TGProfilingInfo, TGWinsockNetwork + 6 networking subclasses. Anchored via factory ID or vtable rather than bare string. **Corrigenda 2026-05-28 (protocol foundation #2):** TGBufferStream and TGMessage are BOTH actually SWIG-visible. TGBufferStream is at vtable 0x00895C58 (ctor FUN_006CEFE0, 0x30 bytes). TGMessage is at vtable 0x008958D0 (ctor FUN_006B82A0, 0x40 bytes) — this vtable was previously mis-attributed to TGBufferStream. See `docs/protocol/stream-primitives.md` § "Two Stream Classes" for the resolution.
...
### 2026-05-28 — decompiled-functions.md (leaf #10 — FINAL engine doc) — **fourth `verified` doc; engine family complete**

- **Confirmed:** 34 unique function addresses + 5 decompile-verified behavioral spot-checks + 7 cross-doc anchors + 4 event IDs — all PASS. (1) Address existence: 34 of 34 unique function addresses present as functions in current Ghidra import (no `dispatcher-recovery`-style missing surprises). (2) Behavioral spot-checks: InitMultiplayer `FUN_00445d90` (WSN/NetFile/GameSpy subobject construction pattern + offsets +0x78/+0x80/+0x7C confirmed); NetFile ctor `FUN_006a30c0` (3 hash tables A/B/C capacity 0x25, vtables `0x895648`/`0x895634`/`0x895620`, byte-anchored `PUSH 0x60001 ; MOV ECX,0x97f864 ; CALL 0x006db380` at `0x006a31ba-c9`); NetFile::ReceiveMessageHandler `FUN_006a3cd0` (opcode dispatch 0x20-0x27 + stream-type guard via vtable[0]==0x32 TGBufferStream tag + `g_bMpgameInOpcodeDispatch` re-entry guard); ChecksumCompleteHandler `FUN_006a1b10` (two 0x40-byte TGBufferStreams via `FUN_00717b70(0x40)`, reliable flag +0x3A=1, sent via FUN_006b4c10); TGNetwork::Update `FUN_006b4560` (3 unconditional sub-calls FUN_006b55b0/FUN_006b5c90/FUN_006b5f70 + state machine flags +0x10C/+0x10D/+0x10E/+0x10F). (3) Cross-doc anchors: MultiplayerGame +0x74 playerSlots[16] stride 0x18 + +0x1F8 readyForNewPlayers + +0x1FC maxPlayers all align with [ui-class-hierarchy.md](ui-class-hierarchy.md); EventManager singleton at 0x0097F838 + handler registry at 0x0097F864 (=+0x2C) match [event-system-architecture.md](event-system-architecture.md); UtopiaModule 0x0097FA00 with WSN+0x78 / NetFile+0x80 / GameSpy+0x7C and Clock 0x009a09d0 with gameTime+0x90 match CLAUDE.md Key Globals; MainTick (FUN_0043b4f0) does NOT call TGNetwork::Update — confirmed by full call-list walk (calls events / timers / 6 subsystems / render, no FUN_006b4560), consistent with [function-map.md](function-map.md)'s SimulationPipelineTick at 0x00451ac0. (4) Event IDs: 0x60001 ET_NETWORK_MESSAGE_EVENT byte-anchored at NetFile ctor and TGNetwork::Update dispatch; 0x60002 HOST event + 0x8000e7 ET_SYSTEM_CHECKSUM_FAILED + 0x8000e8 ET_CHECKSUM_COMPLETE pattern-anchored.
- **Corrected:** 0. The doc was unusually well-anchored — every entry already cited a hex address. Pre-v5 drift was minimal (only stylistic `+0x8a` vs `+0x8A` differences).
- **Dropped:** 0.
- **New sections added:** (a) Top-of-doc NOTE block establishing `status: verified`, naming the 5 decompile-verified entries and the 7 cross-doc anchors, and surfacing the pattern note that address-first authoring correlates with low pre-v5 drift; (b) per-function entry for `FUN_006b9b20 CreateUDPSocket` (was in Quick Reference but missing from per-function sections — small final polish); (c) "Cross-Doc Anchors" footnote table near the end of the doc enumerating the 7 verified cross-doc claims.
- **Pending (confidence: medium):** ~29 of 34 per-function entries are address-confirmed with behavior carried forward from pre-v5 authoring (medium confidence). Pattern uniformity is well-evidenced via the 5 spot-checks. A per-function decompile sweep would promote all 29 to high; tracked as documentation debt. 0 `confidence: low` rows.
- **Open questions:**
  1. **FUN_006b3ec0 HostOrJoin** — doc claims fire event 0x60002 on HOST path; only function existence confirmed, not the event-fire instruction. Low-stakes; would settle with a HostOrJoin decompile.
  2. **FUN_006a4a00 / FUN_006a4bb0** — event-fire claims (0x8000e7 / 0x8000e8) pattern-anchored, not directly verified at their fire sites.
  3. **FUN_006b9b20 CreateUDPSocket** — added as a per-function entry this pass (medium confidence). Per-line behavior carried forward; bind + non-blocking pattern is plausible but unverified at the decompile level.
- **Cross-doc reconciliation:** None. All anchored claims match foundation/mid docs. No corrections needed in companion docs.
- **Status:** `verified`. **Fourth doc in the campaign to reach `verified`** (after nirtti-factory-catalog.md, netimmerse-vtables.md, tg-hierarchy-vtables.md) and the cleanest validation in the engine family — 0 corrections, 0 drops. Every claim has `confidence: high` (5 spot-checked entries, 4 cross-doc anchors verified at byte level, MainTick negative claim) or `confidence: medium` (pattern-extrapolation with documented reasoning, same precedent as netimmerse-vtables.md / nirtti-factory-catalog.md / tg-hierarchy-vtables.md). No `confidence: low` rows.
- **Pattern note for future doc authors:** Pre-v5 drift correlates strongly with prose-first authoring; address-first docs (lead each entry with hex address, prose follows) drift much less. This doc validated cleanly because that's how it was written. Worth adopting as a docwriter convention.
- **Engine family complete:** With doc #10 done, all 10 engine docs are v5-validated. Family breakdown: 5 `verified` (nirtti-factory-catalog.md, netimmerse-vtables.md, tg-hierarchy-vtables.md, decompiled-functions.md, plus the README at refresh time); 5 `partial` (function-map.md, rtti-class-catalog.md, function-mapping-report.md, gamebryo-cross-reference.md, event-system-architecture.md, ui-class-hierarchy.md). README pending an engine-family-close refresh.
- **Engine-family-close batch tasks** (NOT done this pass; flagged for the orchestrator):
  1. **CLAUDE.md Key Globals correction** (per doc #9 ui-class-hierarchy.md finding): row "0x0097e238 TopWindow/MultiplayerGame ptr" → "0x0097e238 PlayWindow / Game state ptr"; add new row "0x009878cc TopWindow (root scene container)".
  2. **CLAUDE.md naming-coverage refresh** (per doc #6 function-mapping-report.md finding): roll back stale "~15,134 (83%)" and "2,348 functions, 393 classes" annotation-script claims to current 25.8% reality.
  3. **docs/engine/README.md index check** — verify all 10 docs are indexed; tg-hierarchy-vtables.md was historically orphan from the index but appears to be present now (added during doc #6 docwriter pass).
  4. **netimmerse-vtables.md NiAVObject 0xC4 → 0xC8 reconciliation** (per doc #7 gamebryo-cross-reference.md finding) — the doc is `verified` but has a now-slightly-wrong NiAVObject size. Reconciliation candidate: bump to 0xC8 with a note that the +4 delta is the helper FUN_008136c0 call at end of ctor.
- **Files touched:** `docs/engine/decompiled-functions.md` (rewritten with v5 frontmatter ~12 evidence rows, top NOTE block, inline `[v5-validated 2026-05-28 — decompile-verified]` tags on 5 spot-checked entries + `[v5-validated 2026-05-28]` on remaining ~29 entries + Quick Reference table header, new FUN_006b9b20 CreateUDPSocket per-function entry, new Cross-Doc Anchors footnote table), `docs/engine/v5-validation-status.md` (this tracker — §2 row #10 status rewritten verified, this log entry added). [docs/engine/README.md](README.md) row for decompiled-functions.md is "Key function analysis (network / checksum / event focus)" — caption remains accurate; **batched to engine-family-close** per campaign convention; not modified this pass. CLAUDE.md doc-index row likewise batched.
